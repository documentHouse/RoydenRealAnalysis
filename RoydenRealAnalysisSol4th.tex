...
\[  m^*(E) - m^*(F) = m^*(E \sim F) = 0 \implies m^*(E) = m^*(F) \]

\noindent We now need to show that if there exists an $F \in F_\sigma$ such that $m^*(E \sim F) = 0$ and a $G \in G_\sigma$ for which $E \subset G$ and $m^*(E) = m^*(G)$ then E is measurable.
