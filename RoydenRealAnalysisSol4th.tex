...
Show that $(i)$ the translate of an $F_\sigma$ set is also $F_\sigma$,$(ii)$ the translate of a $G_\delta$ set is also $G_sigma$ , and (iii) the translate of a set of measure zero also has measure zero.
\\\\ \emph{Proof.}
...
\rule{500pt}{1pt}\\
\end{flushleft} 
Show that if $E$ has finite measure and $\epsilon$, then E is the disjoint union of a finite number of measurable sets, each of which as measure at most $E$.
\\\\ \emph{Proof.}
