...
From the fact that $B \subset A \cup B$ we have from montonocity that $m^*(B) \le m^*(A \cup B)$. We also have by countable subadditivity we have $m^*(A \cup B) \le m^*(A) + m^*(B) = m^*(B)$. These two relationships give $m^*(B) = m^*(A \cup B)$.
